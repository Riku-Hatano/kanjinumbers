...
export const K2n = (kanji: string): returnValue => {
    ...
    if(K2nException(kanji).status) { //入力できない数値をチェックする関数(K2nException)に異常がなかったとき、実際に漢数字からアラビア数字に書き換える処理を実行する
            ...
            if(digitFlag) { //桁の最後に”兆”、”億”、”万”のいずれかがついている場合
                ...
                if(kanjiChunks[i].length % 2 === 0) {
                    ...
                    for(let j = 0 ; j < (kanjiChunks[i].length - 1) / 2 + 1 ; j++) {
                        if(j === (kanjiChunks[i].length - 1) / 2) {
                            chunkNum += numberChars[kanjiChars.indexOf(kanjiChunks[i][kanjiChunks[i].length - 1])];
                        } else {
                            chunkNum += KanjiMultiplyFunc(kanjiChunks[i][j * 2], kanjiChunks[i][j * 2 + 1])
                        }
                    }
